...
def _ocr_clean(text: str) -> str:
    ...
    # 일자 뒤에 바로 붙은 시각 분리: '31일17시' → '31일 17시'
    t = _re.sub(r'(일)(\d)', r'\1 \2', t)
    # 시/분/초 앞뒤 공백 완전 제거: '17 시 25 분 59 초' → '17시25분59초'
    for _ in range(4):
        t = _re.sub(r'(\d)\s*(시|분|초)', r'\1\2', t)
        t = _re.sub(r'(시|분)\s*(\d)', r'\1\2', t)
    ...
    # ── 4단계: 의미 단어 경계 공백 삽입 ────────────────────────────────
    _SPACE_FIXES = [
        # 날짜 뒤 제호 앞 공백 (한글 음절 제거 후 여기서 삽입해야 유지됨)
        (r'(일)(제\d)', r'\1 \2'),
        # 기관명 경계
        (r'(고양지원)(파주등기소)',     r'\1 \2'),
